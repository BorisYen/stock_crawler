...
function updateBBandAll(records, bband_days){
    records = utils.sortRecord(records, true) ;
    ...
    //create these strings for later use.
    for(var i=0; i<bband_days.length; i++){
        ma_days.push('ma'+bband_days[i]) ;
        bband_sdev.push('bband'+bband_days[i]+'_sdev') ;
        bband_b.push('bband'+bband_days[i]+'_b') ;
        bband_width.push('bband'+bband_days[i]+'_width') ;
        bband_up.push('bband'+bband_days[i]+'_up') ;
        bband_low.push('bband'+bband_days[i]+'_low') ;
    }

    for(var i=0; i<bband_days.length; i++){
        for(var j=0; j<records.length; j++){
            if(records[j][ma_days[i]]){
                var tmp = 0 ;

                for(var k=0; k<bband_days[i]; k++){
                    tmp += Math.pow((records[j+k].close - records[j][ma_days[i]]), 2) ;
                }

                records[j][bband_sdev[i]] = Math.sqrt(tmp/bband_days[i]) ;
                records[j][bband_up[i]] = records[j][ma_days[i]]+2*Math.sqrt(tmp/bband_days[i]) ;
                records[j][bband_low[i]] = records[j][ma_days[i]]-2*Math.sqrt(tmp/bband_days[i]) ;
                records[j][bband_b[i]] = (records[j].close - records[j][bband_low[i]])/(records[j][bband_up[i]]-records[j][bband_low[i]]) ;
                records[j][bband_width[i]] = (records[j][bband_up[i]]-records[j][bband_low[i]])/records[j][ma_days[i]] ;
            }
        }
    }
}
...
/**
 * this is based on http://nengfang.blogspot.tw/2015/03/dmi-excel.html
 */
function updateDMIAll(records, dmi_days){
    records = utils.sortRecord(records) ;
    dmi_days = utils.sortDays(dmi_days) ;
    ...
    var tr_attrs_map = utils.getAttrMap('tr', dmi_days) ;
    var pdm_attrs_map = utils.getAttrMap('p_dm', dmi_days) ;
    var ndm_attrs_map = utils.getAttrMap('n_dm', dmi_days) ;
    var tmp_tr_sum = 0 ;
    var tmp_pdm_sum = 0 ;
    var tmp_ndm_sum = 0 ;
    ...
    var pdi_attrs_map = utils.getAttrMap('p_di', dmi_days) ;
    var ndi_attrs_map = utils.getAttrMap('n_di', dmi_days) ;
    ...
    var tmp_dx_sum = 0 ;
    var dx_attrs_map = utils.getAttrMap('dx', dmi_days) ;
    var sum_count = 0 ;
...
function updateMACDAll(records, macd_paras){
    ...
    var ema_attrs = utils.getAttrArr('ema', ema_days) ;
...
function updatePsyAll(records, psy_days){
    records = utils.sortRecord(records, true) ;
    psy_days = utils.sortDays(psy_days) ;
...
function updateBiasAll(records, bias_days){
    records = utils.sortRecord(records) ;
    bias_days = utils.sortDays(bias_days) ;

    var ma_attrs = utils.getAttrArr('ma', bias_days) ;
    var bias_attrs = utils.getAttrArr('bias', bias_days) ;
...
function updateRSIAll(records, rsi_days){
    records = utils.sortRecord(records) ;
    rsi_days = utils.sortDays(rsi_days) ;
...
function updateMaAll(records, mv_days){
    records = utils.sortRecord(records, true) ;
    mv_days = utils.sortDays(mv_days) ;
...
function updateKDAll(records, kd_days){
    records = utils.sortRecord(records) ;
    kd_days = utils.sortDays(kd_days) ;
...
exports.updateMaAll = updateMaAll ;
...
exports.updateBBandAll = updateBBandAll ;