    ...
    return new Promise(function(resolve, reject){
        request.get({
            ...
            if(res.statusCode < 200 || res.statusCode >= 300) {
                // console.log(body) ;
                logger.error(node_util.format('Crawler %s, StatusCode: %d, Options: %j', that.name, res.statusCode, options)) ;
                return reject(new Error(node_util.format('Crawler %s, StatusCode: %d, Options: %j', that.name, res.statusCode, options))) ;
            } 
            ...
            if(data){
                data.forEach(function(item, rindex, arr){
                    try{
                        ...
                        // logger.warn(node_util.format('No data available for crawler %s. Options: %j', e.message, options)) ;
                    }
                }) ;

                that.post_crawl(options, ret)
            } else {
                logger.info(node_util.format('No data available for crawler %s. Options: %j', that.name, options)) ;
            }
 ...
 * the return value is an array of objects.
 * 
 * TODO: v0.2 the twse seems to return http 503 if the program sends requests quickly.
 * Might need to remove crawl data for a whole year here.
 */
